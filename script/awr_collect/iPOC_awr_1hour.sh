...
select to_char(s.startup_time,'dd Mon "at" HH24:mi:ss') instart_fmt
 ...
 , s.snap_level lvl
 , (select SYS_CONTEXT('USERENV', 'HOST', 15) from dual) hostname
...
and  to_char(s.begin_interval_time,'YYYYMMDD HH24:MI')  >   to_char((TRUNC(SYSDATE+1/24,'HH')-4/24 ), 'YYYYMMDD HH24:MI') 
order by di.db_name, i.instance_name, s.snap_id;
begin
 for c1 in c
 loop
 if c1.begin_snap_id > 0 then
 dbms_output.put_line('spool /tmp/'||c1.hostname||'_'||c1.inst_name||'_'
...
mv /tmp/${host}_$ORACLE_SID*.html $AWR_HOME
exit
